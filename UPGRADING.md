...
Refactor](https://github.com/cometbft/cometbft/blob/main/docs/architecture/adr-067-mempool-refactor.md).

## v0.34.0

**Upgrading to CometBFT 0.34 requires a blockchain restart.**
This release is not compatible with previous blockchains due to changes to
the encoding format (see "Protocol Buffers," below) and the block header (see "Blockchain Protocol").

Note also that CometBFT 0.34 also requires Go 1.16 or higher.
...
* New ABCI methods (`ListSnapshots`, `LoadSnapshotChunk`, `OfferSnapshot`, and `ApplySnapshotChunk`)
  ...
  Read [the spec](https://github.com/cometbft/cometbft/blob/v0.34.x/spec/abci/apps.md#state-sync)
...
* The [`PublicKey` type](https://github.com/cometbft/cometbft/blob/main/proto/tendermint/crypto/keys.proto#L13-L15)
  (used in ABCI as part of `ValidatorUpdate`) now uses a `oneof` protobuf type.
  Note that since CometBFT only supports ed25519 validator keys, there's only one
...
CometBFT now relies on the application to tell it which transactions to index. This means that
in the `config.toml`, generated by CometBFT, there is no longer a way to specify which
...
Applications can notify CometBFT to index a specific transaction by setting
...
CometBFT 0.34 replaces Amino with Protocol Buffers for encoding.
This migration is extensive and results in a number of changes, however,
CometBFT only uses the types generated from Protocol Buffers for disk and
wire serialization.
**This means that these changes should not affect you as a CometBFT user.**

However, CometBFT users and contributors may note the following changes:
...
CometBFT 0.34 includes new and updated consensus parameters.
...
* The multisig functionality that was previously in CometBFT now has
...
The `cometbft lite` command has been renamed to `cometbft light` and has a slightly different API.
See [the docs](https://docs.cometbft.com/v0.34/core/light-client-protocol.html#http-proxy) for details.
...
Version is now set through Go linker flags `ld_flags`. Applications that are using CometBFT as a library should set this at compile time.

Example:

```sh
go install -mod=readonly -ldflags "-X github.com/cometbft/cometbft/version.TMCoreSemVer=$(go list -m github.com/cometbft/cometbft | sed  's/ /\@/g') -s -w " -trimpath ./cmd
```
...
ResponseCommit added a field for block retention. The application can provide information to CometBFT on how to prune blocks.
...
keys are called
[compositeKeys](https://github.com/cometbft/cometbft/blob/main/docs/app-dev/indexing-transactions.md).
...
* `/block_results` response format updated [see RPC docs for details](https://docs.cometbft.com/v0.34/rpc/#/Info/block_results)
...
Use `cleveldb` tag instead of `gcc` to compile CometBFT with CLevelDB or
use `make build_c` / `make install_c` (full instructions can be found at
<https://docs.cometbft.com/v0.34/introduction/install.html#compile-with-cleveldb-support>)
...
If your WS client is not consuming events fast enough, CometBFT can terminate the subscription.
...
```json
{
  ...
  "error": {
    "code": -32000,
    "msg": "Server error",
    "data": "subscription was canceled (reason: client is not pulling messages fast enough)" // or "subscription was canceled (reason: CometBFT exited)"
  }
}
...
Any implementations of CometBFT blockchain verification, including lite clients,
will need to be updated. For specific details:

* [Merkle tree](https://github.com/cometbft/cometbft/blob/main/spec/blockchain/encoding.md#merkle-trees)
* [ConsensusParams](https://github.com/cometbft/cometbft/blob/main/spec/blockchain/state.md#consensusparams)

There was also a small change to field ordering in the vote struct. Any
implementations of an out-of-process validator (like a Key-Management Server)
will need to be updated. For specific details:

* [Vote](https://github.com/cometbft/cometbft/blob/main/spec/consensus/signing.md#votes)
...
CometBFT. This is different from how Unix sockets used to work, where
CometBFT dialed the external process.
...
consolidated under the CometBFT semantic version (ie. `node_info.version`) and
