...
func (sc *scheduler) IsPeerBanned(peerID p2p.ID) bool {
	return sc.isPeerBanned(peerID)
}

// CONTRACT: pool.mtx must be locked.
func (sc *scheduler) isPeerBanned(peerID p2p.ID) bool {
	// AGORIC: inline because our cmttime is too old
	// return cmttime.Since(sc.bannedPeers[peerID]) < time.Second*60
	t := sc.bannedPeers[peerID]
	return cmttime.Now().Sub(t) < time.Second*60
}

// CONTRACT: pool.mtx must be locked.
func (sc *scheduler) banPeer(peerID p2p.ID) {
	// pool.logger.Debug("Banning peer", "id", peerID)
	sc.bannedPeers[peerID] = cmttime.Now()
}
...
func (sc *scheduler) setPeerRange(peerID p2p.ID, base int64, height int64) error {
	...
	if base < peer.base || height < peer.height {
		// sc.logger.Info("Peer is reporting height/base that is lower than what it previously reported",
		// 	"peer", peerID,
		...
		sc.banPeer(peerID)
		return fmt.Errorf("peer %s reported base %d height %d lower than previous base %d height %d",
			peerID, base, height, peer.base, peer.height)
	}
...
func (sc *scheduler) handleTryPrunePeer(event rTryPrunePeer) (Event, error) {
	...
	for peerID := range sc.bannedPeers {
		if !sc.isPeerBanned(peerID) {
			delete(sc.bannedPeers, peerID)
		}
	}
