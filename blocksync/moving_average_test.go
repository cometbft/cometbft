...
func TestMovingAverageRingBuffer(t *testing.T) {
	...
	avgs, ok := ma.Add(10 * time.Second) // buffer: [10,2,3]
	require.True(t, ok)
	require.Equal(t, 5*time.Second, avgs) // (10+2+3)/3 = 5

	// Add 5th value — evicts 2.
	avg, ok := ma.Add(20 * time.Second) // buffer: [10,20,3]
	require.True(t, ok)
