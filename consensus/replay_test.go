...
func sendTxs(ctx context.Context, cs *State) {
	for i := 0; i < 256; i++ {
		...
		default:
			...
			reqRes, err := assertMempool(cs.txNotifier).CheckTx(tx, mempool.TxInfo{})
			if err != nil {
				panic(err)
			}
...
func setupChainWithChangingValidators(t *testing.T, name string, nBlocks int) (*cfg.Config, []*types.Block, []*types.ExtendedCommit, sm.State) {
	...
	_, err = assertMempool(css[0].txNotifier).CheckTx(newValidatorTx1, mempool.TxInfo{})
	...
	_, err = assertMempool(css[0].txNotifier).CheckTx(updateValidatorTx1, mempool.TxInfo{})
	assert.NoError(t, err)
	propBlock, err = css[0].createProposalBlock(ctx) // changeProposer(t, cs1, vs2)
	require.NoError(t, err)
	propBlockParts, err = propBlock.MakePartSet(partSize)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTx(newValidatorTx2, mempool.TxInfo{})
	require.NoError(t, err)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTx(newValidatorTx3, mempool.TxInfo{})
	assert.NoError(t, err)
	propBlock, err = css[0].createProposalBlock(ctx) // changeProposer(t, cs1, vs2)
	require.NoError(t, err)
	propBlockParts, err = propBlock.MakePartSet(partSize)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTx(removeValidatorTx2, mempool.TxInfo{})
	assert.Nil(t, err)
	...
	_, err = assertMempool(css[0].txNotifier).CheckTx(removeValidatorTx3, mempool.TxInfo{})
	assert.NoError(t, err)
	propBlock, err = css[0].createProposalBlock(ctx) // changeProposer(t, cs1, vs2)
	require.NoError(t, err)
	propBlockParts, err = propBlock.MakePartSet(partSize)
