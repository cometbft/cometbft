...
func (cs *State) updateToState(state sm.State) {
	...
	case cs.LastCommit == nil:
		// NOTE: when Tendermint consensus starts, it has no votes. reconstructLastCommit
...
func (cs *State) defaultDoPrevote(height int64, round int32) {
	...
	// Validate proposal block, from Tendermint consensus' perspective
	...
	/*
		...
		valid. If the Application does not accept the block, Tendermint consensus prevotes `nil`.

		WARNING: misuse of block rejection by the Application can seriously compromise
		Tendermint consensus' liveness properties.
