...
1. Provide an authentication mechanism for the RPC query endpoints

### RPC Endpoints

These are the planned RPC companion endpoints to be implemented that will mirror the CometBFT RPC endpoints.

The endpoints on this following table are the ones that can be implemented first since it seems it will be straightforward and less complex

| **RPC Companion Endpoint** | **CometBFT Endpoint** | **Parameters**                                   | **Description**                                 | **Notes**                                                                                                                                                                                                                                                                               |
|----------------------------|-----------------------|--------------------------------------------------|-------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `/v1/abci_info`            | `/abci_info`          |                                                  | Get information about the application           | This companion endpoint will return the same response structure as the CometBFT endpoint. The companion endpoint will return the latest information stored in its database that was retrieved from the full node.                                                                       |
| `/v1/block`                | `/block`              | * height                                         | Get block at a specified height                 | This companion endpoint will return the same response structure as the CometBFT endpoint. The data retrieved from the companion database for a particular block will have to be properly serialized into the `block` struct in order to be returned as a response.                      |
...
| `/v1/blockchain`           | `/blockchain`         | * minHeight <br/> * maxHeight                    | Get blocks in a specified height range          | This companion endpoint will return the same response structure as the CometBFT endpoint. The data retrieved from the companion database will include one or more blocks.                                                                                                               |
| `/v1/commit`               | `/commit`             | * height                                         | Get commit results at a specified height        | This companion endpoint will return the same response structure as the CometBFT endpoint.                                                                                                                                                                                               |
| `/v1/consensus_params`     | `/consensus_params`   | * height                                         | Get consensus parameters at a specified height  | This companion endpoint will return the same response structure as the CometBFT endpoint.                                                                                                                                                                                               |
| `/v1/header`               | `/header`             | * height                                         | Get header at a specified height                | This companion endpoint will return the same response structure as the CometBFT endpoint.                                                                                                                                                                                               |
| `/v1/header_by_hash`       | `/header_by_hash`     | * hash                                           | Get header by its hash                          | This companion endpoint will return the same response structure as the CometBFT endpoint.                                                                                                                                                                                               |
...
| `/v1/tx`                   | `/tx`                 | * hash <br/> * prove                             | Get a transaction by its hash                   | This companion endpoint will return the same response structure as the CometBFT endpoint.                                                                                                                                                                                               |
| `/v1/validators`           | `/validators`         | * height <br/> * page <br/> * per_page           | Get validator set at a specified height         | This companion endpoint will return the same response structure as the CometBFT endpoint.                                                                                                                                                                                               |

The following endpoints can also be implemented, but might require some additional effort and complexity to be implemented. These are mostly the `_search_` and `_query_` endpoints.

| **RPC Companion Endpoint** | **CometBFT Endpoint** | **Parameters**                                                       | **Description**                        | **Notes**                                                                                                                                                                                    |
|----------------------------|-----------------------|----------------------------------------------------------------------|----------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `/v1/abci_query`           | `/abci_query`         | * path <br/> * data <br/> * height <br/> * prove                     | Query information from the application | This companion endpoint will return the same response structure as the CometBFT endpoint. The RPC companion service will have to implement a proper abci parameter to sql query translation  |
| `/v1//block_search`        | `/block_search`       | * query <br/> * page <br/> * per_page <br/> * order_by               | Query information about a block        | This companion endpoint will return the same response structure as the CometBFT endpoint. The RPC companion service will have to implement a proper query parameter to sql query translation |
| `/v1//tx_search`           | `/tx_search`          | * query <br/> * page <br/> * per_page <br/> * prove <br/> * order_by | Query information about transactions   | This companion endpoint will return the same response structure as the CometBFT endpoint. The RPC companion service will have to implement a proper query parameter to sql query translation |

These endpoints will proxy the request to the full node. Since they are not dependent on data retrieval from the companion database they should just act as proxies to the full node.

This will ensure that clients don't need to implement a routing logic for endpoints that would not be available in the RPC companion service.
...
| **RPC Companion Endpoint** | **CometBFT Endpoint**   | **Parameters**                                         | **Description**                           | **Notes**                                                                                                                                                                                                                                       | Proxy |
|----------------------------|-------------------------|--------------------------------------------------------|-------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------|
| `/v1/broadcast_evidence`   | `/broadcast_evidence`   | * evidence                                             | Broadcast evidence of the misbehavior     | The evidence parameter is in JSON format                                                                                                                                                                                                        | yes   |
| `/v1/broadcast_tx_async`   | `/broadcast_tx_async`   | * tx                                                   | Broadcast a transaction                   | Returns right away with no response                                                                                                                                                                                                             | yes   |
| `/v1/broadcast_tx_sync`    | `/broadcast_tx_sync`    | * tx                                                   | Broadcast a transaction                   | Returns with the response from CheckTx                                                                                                                                                                                                          | yes   |
| `/v1/broadcast_tx_commit`  | `/broadcast_tx_commit`  | * tx                                                   | Broadcast a transaction                   | Returns with the responses from CheckTx and DeliverTx                                                                                                                                                                                           | yes   |
| `/v1/check_tx`             | `/check_tx`             | * tx                                                   | Check a transaction                       | Checks a transaction without executing it                                                                                                                                                                                                       | yes   |
| `/v1/consensus_state`      | `/consensus_state`      |                                                        | Gets consensus state                      | The consensus state will not be stored in the RPC companion database so it should proxy the request to the full node                                                                                                                            | yes   |
| `/v1/dump_consensus_state` | `/dump_consensus_state` |                                                        | Gets the full consensus state             | The consensus state will not be stored in the RPC companion database so it should proxy the request to the full node                                                                                                                            | yes   |
| `/v1/genesis`              | `/genesis`              |                                                        | Gets the genesis information              | The RPC companion service can proxy the genesis request to the full node. If there are use cases that serving the genesis from the RPC companion service (no proxy) is desirable then it can be implemented as a RPC companion service endpoint | yes   |
| `/v1/net_info`             | `/net_info`             |                                                        | Gets network information                  | The request should proxy to the full node since the RPC companion database will not store network information                                                                                                                                   | yes   |
| `/v1/unconfirmed_txs`      | `/unconfirmed_txs`      | * limit                                                | Gets the list of unconfirmed transactions | The request should proxy to the full node since the RPC companion database will not store unconfirmed transactions information                                                                                                                  | yes   |
| `/v1/num_unconfirmed_txs`  | `/num_unconfirmed_txs`  |                                                        | Gets data about unconfirmed transactions  | The request should proxy to the full node since the RPC companion database will not store unconfirmed transactions information                                                                                                                  | yes   |
| `/v1/status`               | `/status`               |                                                        | Gets node status                          | The request should proxy to the full node since the RPC companion database will not store node status information                                                                                                                               | yes   |
...
transactions with sequence mismatch errors. These requests will proxy to the full node as outlined above.
