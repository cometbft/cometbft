...
In ABCI++, the `Commit` method is used to persist the changes made to the application state during the FinalizeBlock method.
...
*In the next session, you will learn about how user can [**Query Message**](./4.query-message.md) in ABCI++ Forum Application.*
