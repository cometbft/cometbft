...
# Migrating from CometBFT v0.38.x to v1.0

This document is intended to provide detailed insights into significant changes that may
require additional information beyond what is covered in the [CHANGELOG.md](https://github.com/cometbft/cometbft/blob/main/CHANGELOG.md)
or [UPGRADING.md](https://github.com/cometbft/cometbft/blob/main/UPGRADING.md) documents in regard to the necessary changes
for migrating from CometBFT `v0.38.x` to `v1.0`.
...
In the `v1.0` release, two additional local ABCI clients have been added.

1.The first, supports one different mutex per ABCI connection
...
2.The second, totally removes mutexes from the ABCI client.
...
If you are uncertain about how to ensure these guarantees in your application,
it is strongly recommended to continue using the existing ABCI client, which contains
...
definitions:
  ...
  path-compatible) with CometBFT `v0.34.x`, ``v0.37.x`` and `v0.38.x`, where breaking changes
  were introduced, are available under `v1beta*`-versioned types.
    - For example:

      - The `tendermint.abci.Request` type from CometBFT `v0.34.x` is now available as
     `cometbft.abci.v1beta1.Request`.
      - The `tendermint.abci.Request` type from CometBFT `v0.37.x` is now available as
     `cometbft.abci.v1beta2.Request`.
      - The `tendermint.abci.Request` type from CometBFT `v0.38.x` is now available as
...
- The RPC API is now versioned, with the existing RPC being available under both
  the `/` path (as in CometBFT `v0.38.x`) and a `/v1` path.
...
Review the [official CHANGELOG.md](https://github.com/cometbft/cometbft/blob/main/CHANGELOG.md) for detailed information on changes and improvements in `v1.0`.
