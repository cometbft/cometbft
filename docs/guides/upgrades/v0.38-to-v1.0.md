...
- The first adopts one mutex per ABCI connection
...
When using either of the new ABCI clients, the application is now responsible to coordinate concurrent ABCI
calls in order to prevent race conditions or the possibility of non-determinism.

If you are uncertain about how to ensure these guarantees in your application, it is strongly recommended
to continue using the existing ABCI client, which contains a single global mutex.
