...
func TestApplyBlock(t *testing.T) {
	...
	state, stateDB, _ := test.MakeState(1, 1, chainID)
...
func TestFinalizeBlockDecidedLastCommit(t *testing.T) {
	...
	state, stateDB, privVals := test.MakeState(7, 1, chainID)
	stateStore := sm.NewStore(stateDB, sm.StoreOptions{
		DiscardABCIResponses: false,
	})
...
func TestFinalizeBlockValidators(t *testing.T) {
	...
	state, stateDB, _ := test.MakeState(2, 2, chainID)
	stateStore := sm.NewStore(stateDB, sm.StoreOptions{
		DiscardABCIResponses: false,
	})
...
func TestFinalizeBlockMisbehavior(t *testing.T) {
	...
	state, stateDB, privVals := test.MakeState(1, 1, chainID)
	stateStore := sm.NewStore(stateDB, sm.StoreOptions{
		DiscardABCIResponses: false,
	})
...
func TestProcessProposal(t *testing.T) {
	...
	state, stateDB, privVals := test.MakeState(1, height, chainID)
	stateStore := sm.NewStore(stateDB, sm.StoreOptions{
		DiscardABCIResponses: false,
	})
	blockStore := store.NewBlockStore(dbm.NewMemDB())
...
func TestCreateProposalAbsentVoteExtensions(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			state, stateDB, privVals := test.MakeState(1, int(testCase.height-1), chainID)
			stateStore := sm.NewStore(stateDB, sm.StoreOptions{
				DiscardABCIResponses: false,
			})
