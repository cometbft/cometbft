...
func (sc *RetrySignerClient) SignOracleVote(chainID string, vote *oracleproto.GossipedVotes, sigPrefix []byte) error {
	var err error
	for i := 0; i < sc.retries || sc.retries == 0; i++ {
		err = sc.next.SignOracleVote(chainID, vote, sigPrefix)
		if err == nil {
			return nil
		}
		// If remote signer errors, we don't retry.
