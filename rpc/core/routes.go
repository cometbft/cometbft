...
func (env *Environment) GetRoutes() RoutesMap {
	return RoutesMap{
		...
		"data_commitment":           rpc.NewRPCFunc(env.DataCommitment, "start,end"), // <celestia-core />
		"check_tx":                  rpc.NewRPCFunc(env.CheckTx, "tx"),
		"tx":                        rpc.NewRPCFunc(env.Tx, "hash,prove", rpc.Cacheable()),
		"prove_shares":              rpc.NewRPCFunc(env.ProveShares, "height,startShare,endShare"),  // <celestia-core />
		"data_root_inclusion_proof": rpc.NewRPCFunc(env.DataRootInclusionProof, "height,start,end"), // <celestia-core />
