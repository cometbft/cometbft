...
https://github.com/cometbft/cometbft/tree/v2.x/test/loadtime/cmd/report).
    ...
    for input_file in input_files:
        ...
        with open(input_file, 'rt') as inf:
            ...
            for tx in reader:
                ...
                experiments_lane = process_tx_lane(experiments_lane, tx)


    avg_latencies, throughput_rates = compute_experiments_stats(experiments)
    avg_latencies_lane, throughput_rates_lane = compute_lane_stats(experiments_lane)

    return avg_latencies, throughput_rates, avg_latencies_lane, throughput_rates_lane


def process_tx_lane(experiments_lane, tx):
    ...
    if (exp_id,lane) not in experiments_lane:
        experiments_lane[(exp_id,lane)] = {
        ...
        logging.info('Found experiment %s ,lane=%d with rate=%d, connections=%d' %
                     (exp_id, lane, rate, connections))
    else:
        # Validation
        for field in ['connections', 'rate']:
            val = int(tx[field])
            if val != experiments_lane[(exp_id,lane)][field]:
                raise Exception(
                    'Found multiple distinct values for field '
                    '"%s" for the same experiment (%s): %d and %d' %
                    (field, exp_id, val, experiments_lane[(exp_id,lane)][field]))

        if block_time < experiments_lane[(exp_id,lane)]['block_time_min']:
            experiments_lane[(exp_id,lane)]['block_time_min'] = block_time
            experiments_lane[(exp_id,lane)]['block_time_min_duration'] = duration
        if block_time > experiments_lane[(exp_id,lane)]['block_time_max']:
            experiments_lane[(exp_id,lane)]['block_time_max'] = block_time

        experiments_lane[(exp_id,lane)]['total_latencies'] += duration
        experiments_lane[(exp_id,lane)]['tx_count'] += 1

    return experiments_lane
...
def compute_lane_stats(experiments):
    ...
    for (exp_id,lane), exp in experiments.items():
    ...
    for l in lanesSorted:
        statsLane[l] = sorted(statsLane[l], key=lambda s: s['throughput_rate'])
        ...
        for s in statsLane[l]:
            ...
            logging.info('For lane %d: '
                         'throughput rate = %.6f tx/s\t'
                         'average latency = %.6fs' %
                         (l, s['throughput_rate'], s['avg_latency']))

    return (avg_latencies_lane, throughput_rates_lane)
