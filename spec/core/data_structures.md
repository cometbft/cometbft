...
| Time              | [Time](#time)             | The timestamp can be computed using [PBTS][pbts] or [BFT Time][bfttime] algorithms. In case of PBTS, it is the time at which the proposer has produced the block. In case of BFT Time, it is equal to the weighted median of timestamps validators present in the last commit.                                                                                                  | Time must be larger than the Time of the previous block header. The timestamp of the first block should not be smaller than the genesis time. When BFT Time is adopted, it should match the genesis time (since there's no votes to compute the median). |
