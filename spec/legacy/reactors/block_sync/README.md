...
CometBFT full nodes run the BlockSync Reactor as a service to provide blocks
to new nodes. New nodes run the BlockSync Reactor in "block_sync" mode,
where they actively make requests for more blocks until they sync up.
Once caught up, "block_sync" mode is disabled and the node switches to
