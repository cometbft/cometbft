...
`run-multiple.sh` saves each run's manifest, runner output, Docker/Compose versions,
and Docker events under `logs/` (override with `E2E_LOG_DIR`). On failure, it also
saves container state and node logs before cleanup. The PR E2E workflow uploads
these files as diagnostic artifacts.
